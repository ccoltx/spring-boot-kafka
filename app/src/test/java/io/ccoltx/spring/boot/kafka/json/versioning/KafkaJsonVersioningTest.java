...
import io.ccoltx.spring.boot.kafka.json.versioning.data.v1.TestObjectV1;
import io.ccoltx.spring.boot.kafka.json.versioning.data.v2.TestObjectV2;
...
@SpringBootTest
@Testcontainers
@ActiveProfiles("kafka-json-versioning")
class KafkaJsonVersioningTest {

    @Container
    private static final KafkaContainer kafkaContainer = new KafkaContainer(DockerImageName.parse("confluentinc/cp-kafka:5.4.3"));

    @Autowired
    private JsonVersioningProducer producer;

    @SpyBean
    private JsonVersioningConsumer consumer;
